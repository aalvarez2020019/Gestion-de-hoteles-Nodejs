...
function buscarHospedajes(req, res) {
  ...
  let params = req.body;
    Hoteles.findOne({ administradorHotel: req.user.sub },(err, hotelAdmin) => {
        ...
        Usuarios.findOne({$and: [{ usuario: params.buscar }, { hotelHospedado: hotelAdmin._id }],}).exec((err, eventoEncontrado) => {
            if (err)
...
function eliminarHabitacion(req, res) {
  ...
  if (req.user.rol == "ROL_ADMIN_HOTEL") {
    habitacionID = req.params.habitacionID;
    Usuario = req.params.idHotel;
  } else if (req.user.rol == "ROL_ADMIN_HOTEL") {
    Usuario = req.user.sub;
    habitacionID = req.params.habitacionID;
  }
  Habitaciones.findById({ _id: idHotel }, (err, habitacionEncontrada) => {
    if (req.user.rol == "ROL_ADMIN_HOTEL" && habitacionEncontrada.Usuario != req.user.sub)
      return res.status(500).send({ message: "No posee los permisos para eliminar esta habitacion" });

    Habitaciones.findByIdAndDelete({ _id: idHotel, Usuario: Usuario },{ new: true },(err, habitacionEliminada) => {
        if (err) return res.status(500).send({ message: "error al eliminar" });
        if (!habitacionEliminada)
          return res.status(500).send({ message: "No se puedieron eliminar los campos" });

        return res.status(200).send({ Equipo: habitacionEliminada });
      }
    );
  });
}
...
function editarEvento(req, res) {
  ...
  if (req.user.rol == 'ROL_ADMIN_HOTEL' || idUser === req.user.sub) {
      Eventos.findById(idEvento, (err, eventoEncontrado) => {
          if (eventoEncontrado.rol == 'ROL_USUARIO' && req.user.sub === idUser) {
              parametros.rol = 'ROL_USUARIO';
              Eventos.findByIdAndUpdate(idUser, parametros, { new: true }, (err, eventoEditado) => {
                  if (err) return res.status(500).send({ mensaje: 'Error en la peticion' });
                  if (!eventoEditado) return res.status(404).send({ mensaje: 'Error al Editar el evento' })
                  return res.status(200).send({ usuario: eventoEditado });
              })
          } else if (eventoEncontrado.rol == 'ROL_ADMIN_HOTE' && req.user.rol == 'ROL_ADMIN') {
            Eventos.findByIdAndUpdate(idEvento, parametros, { new: true }, (err, eventoEditado) => {
                  if (err) return res.status(500).send({ mensaje: 'Error en la peticion' });
                  if (!eventoEditado) return res.status(404).send({ mensaje: 'Error al Editar el evento' })
                  return res.status(200).send({ usuario: eventoEditado });
              })
...
function eliminarEvento(req, res) {
  var idEvento = req.params.idEvento;
  if (idEvento === req.user.sub || req.user.rol == 'ROL_ADMIN_HOTEL') {

      Eventos.findById(idEvento, (err, eventoEncontrado) => {
          if (eventoEncontrado.rol == 'ROL_USUARIO' && req.user.sub === idEvento) {
            Eventos.findByIdAndDelete(idEvento, (err, eventoEliminado) => {
                  if (err) return res.status(500).send({ mensaje: 'Error en la peticion' });
                  if (!eventoEliminado) return res.status(404).send({ mensaje: 'Error al Eliminar el usuario' })
                  return res.status(200).send({ Usuario: eventoEliminado });
              })
          } 
      })
  }
