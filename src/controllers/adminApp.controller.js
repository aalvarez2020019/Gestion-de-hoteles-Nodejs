...
// Contador de hoteles
/*function HotelesContando(req, res) {

  let contador = 0;
  let parametros = req.body;

  if (parametros.nombre) {
      Usuarios.findOne({ nombre: parametros.nombre }, (err, hotelEncontrado) => {
          if (err) return res.status(500).send({ mensaje: 'Error en la peticion' });
          if (underscore.isEmpty(hotelEncontrado)) return res.status(404).send({ mensaje: 'Error al obtener el hotel' });

          Hoteles.find({ administradorHotel: hotelEncontrado._id, disponible: true }, (err, habitacioDisponible) => {
